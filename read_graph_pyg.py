...
import numpy as np
from ogb.io.read_graph_raw import read_csv_heterograph_raw, read_binary_graph_raw, read_binary_heterograph_raw
from read_graph_raw import read_csv_graph_raw # TODO
...
def read_graph_pyg(raw_dir, add_inverse_edge = False, additional_node_files = [], additional_edge_files = [], binary = False):
    ...
    for graph in tqdm(graph_list):
        ...
        ## TODO: add pos
        if graph['pos_feat'] is not None:
            g.pos = torch.from_numpy(graph['pos_feat'])
            del graph['pos_feat']
        else:
            print("Without pos feature!!!")
