...
def read_csv_graph_raw(raw_dir, add_inverse_edge = False, additional_node_files = [], additional_edge_files = []):
    ...
    ## TODO: add smiles for the graph list
    try:
        smiles = pd.read_csv(osp.join(raw_dir, '../mapping/mol.csv.gz'), compression='gzip', header = None, skiprows=1)[128].astype(str)
        # print(smiles.shape)
        # print(smiles[0])
    ...
    ## TODO: add pos feature
    try:
        pos_feat = pd.read_csv(osp.join(raw_dir, 'add_coord.csv'), header = None).values[:,-3:]       
        pos_feat = pos_feat.astype(np.float32)
    except FileNotFoundError:
        pos_feat = None
    ...
    for num_node, num_edge in tqdm(zip(num_node_list, num_edge_list), total=len(num_node_list)):
        ...
        ## TODO: add smiles
        graph['smiles'] = smiles[num_smiles_accum]
        num_smiles_accum = num_smiles_accum + 1
        ...
        if pos_feat is not None:
            graph['pos_feat'] = pos_feat[num_node_accum:num_node_accum+num_node]
        else:
            graph['pos_feat'] = None
