...
def xyz_to_dat(pos, edge_index, num_nodes):
    """
    ...
    # TODO: store angles and torsions as atom & edge features.
    # angle (idx_j)
    unique_values, counts = torch.unique(idx_j, return_counts=True)
    counts = counts // 2 # there are repeat values for node j
    angle_feature = torch.zeros((pos.shape[0],torch.max(counts)))
    angle_accum = 0
    for t in range(unique_values.shape[0]):
        angle_feature[unique_values[t],:counts[t]] = angle[angle_accum:(angle_accum + counts[t])]
        angle_accum = angle_accum + counts[t]

    # torsion (idx_j,idx_i)
    torsion_feature = torch.zeros((pos.shape[0],pos.shape[0],2)) # TODO: default max torsion num of an edge is 2
    for t in range(torsion.shape[0]):
        if torsion_feature[idx_j[t],idx_i[t],0] == 0:
            torsion_feature[idx_j[t],idx_i[t],0] = torsion[t]
        else:
            torsion_feature[idx_j[t],idx_i[t],1] = torsion[t]

    return angle_feature, torsion_feature