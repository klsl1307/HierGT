...
# Main change:
# 1. edge encoding use rel_pos from SPD (not 3d coords)
# 2. add angle & torsion
...
class Graphormer_0904(pl.LightningModule):
    def __init__(
        ...
        ## TODO
        self.distance_encoder = NonLinear(1, num_heads) # expand the last dimension
        self.angle_encoder = NonLinear(6, hidden_dim) # n_graph * n_node * 6 -> n_graph * n_node * hidden_dim
        self.torsion_encoder = NonLinear(2, num_heads) # n_graph * n_node * n_node * 2 -> n_graph * n_node * n_node * head
        self.use_angle = use_angle
        self.use_torsion = use_torsion

    def forward(self, batched_data, perturb=None):

        attn_bias, rel_pos, x = batched_data.attn_bias, batched_data.rel_pos, batched_data.x # this rel_pos is from 2D SPD
        ...
        coord_rel_pos = delta_pos.norm(dim=-1).view(-1, pos.shape[1], pos.shape[1]) # this rel_pos is from 3D coords
        delta_pos /= coord_rel_pos.unsqueeze(-1) + 1e-5 # normalize
        ...
        ## TODO 3d rel pos, [n_graph, n_node, n_node, n_head] -> [n_graph, n_head, n_node, n_node]
        device = torch.cuda.current_device()
        coord_rel_pos = coord_rel_pos.to(device) # [n_graph, n_node, n_node]

        rel_pos_bias = self.distance_encoder(coord_rel_pos.unsqueeze(-1)) # expand 1 dimension
        ...
        #### TODO: generate angle and torsion features
        # get edge index according to attn_edge_type
        max_angle_feature = 6
        max_torsion_feature = 2
        angle_feature = torch.zeros((n_graph,n_node,max_angle_feature))
        torsion_feature = torch.zeros((n_graph,n_node,n_node,max_torsion_feature))
        for i in range(n_graph):
            edge_index_all = torch.nonzero(attn_edge_type[i,:,:,0])

            ## TODO: if no edge (like 'N'), angle & torsion set as 0 (this may not be good)
            if edge_index_all.shape[0]==0:
                angle = torch.zeros((n_node,max_angle_feature))
                torsion = torch.zeros((n_node,n_node,2))
            else:
            # first dimension: num of atoms N_atom
            # angle: N_atom * 3, torsion: N_atom * N_atom * 2
                angle, torsion = xyz_to_dat(pos[i], torch.t(edge_index_all), torch.max(edge_index_all)+1)

            # angle_bias with node_feature, torsion_bias with edge_feature
            pad = nn.ZeroPad2d(padding=(0,max_angle_feature-angle.shape[1],0,0))
            angle_feature[i] = pad(angle)
            torsion_feature[i] = torsion

        # graph_attn_bias
        ###### TODO: main change!!!
        if self.use_torsion:
            torsion_feature = torsion_feature.to(device)
            torsion_bias = self.torsion_encoder(torsion_feature)
            torsion_bias = torsion_bias.permute(0, 3, 1, 2).contiguous()
            graph_attn_bias[:, :, 1:, 1:] = graph_attn_bias[:,
                                                        :, 1:, 1:] + rel_pos_bias + torsion_bias  # spatial encoder + torsion encoder
        else:
            graph_attn_bias[:, :, 1:, 1:] = graph_attn_bias[:,
        ...
        # edge feature
        if self.edge_type == 'multi_hop': # TODO: use the 2d SPD rel_pos, not 3d coords!!!
        ...
        ###### TODO: main change!!!
        if self.use_angle:
            angle_feature = angle_feature.to(device)
            angle_bias = self.angle_encoder(angle_feature)
            node_feature = node_feature + angle_bias # add angle encoder
